...
records=[];outputs={}
for label in ['clean','poison_legacy','big_endian','incomplete_core','schema','potential_rank',
              'false_precision','integer_state']:
    ...
    with h5py.File(case/saved,'r+') as f:
        ...
        elif label=='big_endian':
            for key in ['ns','Ts','vs1','Phi']:
                path='restart_core/'+key
                data=f[path][...].astype('>f8')
                del f[path];f.create_dataset(path,data=data)
        ...
        elif label in ['false_precision','integer_state']:
            data=f['restart_core/ns'][...].astype('float32' if label=='false_precision' else 'int64')
    ...
    expected={'incomplete_core':'Incomplete core restart','schema':'Unsupported core restart',
              ...
              'integer_state':'must contain float64'}
...
exact=False
if 'clean' in outputs and 'poison_legacy' in outputs:
    exact=all(np.array_equal(outputs['clean'][k],outputs['poison_legacy'][k]) for k in outputs['clean'])
endian_equal=False
if 'clean' in outputs and 'big_endian' in outputs:
    endian_equal=all(np.array_equal(outputs['clean'][k],outputs['big_endian'][k]) for k in outputs['clean'])
...
result=dict(schema='gemini.qualification.restart_core.1',runs=records,legacy_poison_bitwise_equal=exact,dtypes=precision,
            big_endian_bitwise_equal=endian_equal,
            passed=all(r['passed'] for r in records) and exact and endian_equal and set(precision.values())=={'float64'},
