...
rtol=1e-8; rtolN=rtol; rtolT=rtol; rtolJ=rtol;   rtolV=rtol; 
atol=0;                             atolJ=atol; atolV=atol;    
...
[ymd0,UTsec0,tdur,dtout,flagoutput,mloc]=readconfig([dir1,filesep,'inputs/config.ini']);
%% load grid
...
for it=1:Nt
  [neA,v1A,TiA,TeA,J1A,v2A,v3A,J2A,J3A] = loadframe(dir1,times,UTsec,UTsec0,ymd,ymd0,xg,flagoutput,mloc);
  [neB,v1B,TiB,TeB,J1B,v2B,v3B,J2B,J3B] = loadframe(dir2,times,UTsec,UTsec0,ymd,ymd0,xg,flagoutput,mloc);
  
  assert_allclose(neA,neB,rtolN,atol,["Ne accuracy, time #",int2str(it)],false,true)
  
  assert_allclose(v1A,v1B,rtolV,atolV,["V1 accuracy, time #",int2str(it)],false,true)
  assert_allclose(v2A,v2B,rtolV,atolV,["V2 accuracy, time #",int2str(it)],false,true)
  assert_allclose(v3A,v3B,rtolV,atolV,["V3 accuracy, time #",int2str(it)],false,true)
  
  assert_allclose(TiA,TiB,rtolT,atol,["Ti accuracy, time #",int2str(it)],false,true)
  assert_allclose(TeA,TeB,rtolT,atol,["Te accuracy, time #",int2str(it)],false,true)
  
  assert_allclose(J1A,J1B,rtolJ,atolJ,["J1 accuracy, time #",int2str(it)],false,true)
  assert_allclose(J2A,J2B,rtolJ,atolJ,["J2 accuracy, time #",int2str(it)],false,true)
  assert_allclose(J3A,J3B,rtolJ,atolJ,["J3 accuracy, time #",int2str(it)],false,true)
