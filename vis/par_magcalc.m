...
%PULL NECESSARY GRID DATA OUT OF STRUCTURE
proj_er_e1=dot(xg.er,xg.e1,4);
proj_er_e2=dot(xg.er,xg.e2,4);
proj_er_e3=dot(xg.er,xg.e3,4);
proj_etheta_e1=dot(xg.etheta,xg.e1,4);
proj_etheta_e2=dot(xg.etheta,xg.e2,4);
proj_etheta_e3=dot(xg.etheta,xg.e3,4);
proj_ephi_e1=dot(xg.ephi,xg.e1,4);
proj_ephi_e2=dot(xg.ephi,xg.e2,4);
proj_ephi_e3=dot(xg.ephi,xg.e3,4);
xgalt=xg.alt;


%FLUSH STRUCTURE TO SAVE MEMORY (NECESSARY FOR LARGE-ISH SIMULATIONS)
fprintf('Flushing grid...\n');
clear xg;
...
for it=1:lt
    autoload=0;      %don't automatically load the grid!!!!
    loadframe_wrapper;
    clear ne Ti vi1 vi2 vi3;
    fprintf('Flushing unused vars...\n');
...
%    Jphi=J1.*dot(xg.ephi,xg.e1,4)+J2.*dot(xg.ephi,xg.e2,4)+J3.*dot(xg.ephi,xg.e3,4);
    Jr=J1.*proj_er_e1+J2.*proj_er_e2+J3.*proj_er_e3;
    Jtheta=J1.*proj_etheta_e1+J2.*proj_etheta_e2+J3.*proj_etheta_e3;
    Jphi=J1.*proj_ephi_e1+J2.*proj_ephi_e2+J3.*proj_ephi_e3;
