function h=plotslice3D_curv_north_scattered(t,dmy,xg,parm,parmlbl,caxlims)
...
%parmp=interp3(xg.x2(inds2),x3interp,xg.x1(inds1),parmtmp,pplot,phiplot,qplot);
...
parmtmp=permute(parm,[3,2,1]);
%Finterp=TriScatteredInterp(xg.phi(:),xg.theta(:),xg.r(:),parmtmp(:));
scatteredInterpolant(xg.phi(:),xg.theta(:),xg.r(:),parmtmp(:));
parmp=Finterp(phivals(:),thetavals(:),rvals(:));
parmp=reshape(parmp,lyp,lxp,lzp);
