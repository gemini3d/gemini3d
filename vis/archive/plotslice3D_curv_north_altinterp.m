function h=plotslice3D_curv_north_altinterp(t,dmy,xg,parm,parmlbl,caxlims)
...
%INTERPOLATION BASED ON GRID POINT NUMBER
parmtmp=permute(parm,[3,2,1]);
qindplot=interp1(xg.x1(inds1),1:lx1,qplot);
pindplot=interp1(xg.x2(inds2),1:lx2,pplot);
phiindplot=interp1(xg.x3(inds3),1:lx3,phiplot);
parmp=interp3(1:lx3,1:lx2,1:lx1,parmtmp,pindplot,phiindplot,qindplot,'nearest');
parmp=reshape(parmp,lyp,lxp,lzp);    %slice expects the first dim. to be "y"
