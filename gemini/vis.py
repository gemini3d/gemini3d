...
def plot3D_curv_frames_long(
    time: datetime, grid: typing.Dict[str, np.ndarray], parm: np.ndarray, name: str, fg
):
    pass
...
def plot12(x: np.ndarray, z: np.ndarray, parm: np.ndarray, fg, ax=None):
    if parm.ndim != 2:
...
def plot13(y: np.ndarray, z: np.ndarray, parm, fg, ax=None):
    if parm.ndim != 2:
        raise ValueError(f"data must have 2 dimensions, you have {parm.shape}")
    if ax is None:
        ax = fg.gca()
...
def plot_interp(grid: typing.Dict[str, np.ndarray], parm: np.ndarray, fg):
    """
    ...
    if grid["lx"][2] == 1:  # alt./lon. slice
        ...
        # %% CONVERT ANGULAR COORDINATES TO MLAT,MLON
        i = np.argsort(yp)
        plot13(yp[i] * R_EARTH, zp, parmp[:, i], fg)
        ...
        ix = np.argsort(xp)
        iy = np.argsort(yp)
        plot12(xp[ix], zp, parmp[:, ix], fg, axs[0])
        ...
        plot13(xp[ix], yp[iy], parmp[:, ix, 2], fg, axs[1])
        # %% ALT/LAT SLICE
        fmp = interp.interp2d(
            grid["x3"][inds3], grid["x1"][inds1], parm[:, lx2 // 2, :]
        )
        parmp = fmp(x3plot, x1plot)
        plot13(yp[iy], zp, parmp[:, iy], fg, axs[2])
