...
use phys_consts, only: wp, lsp, amu, kb, qs, ln, ms, gammas, elchrg, mn, gamman
...
!                    -pressure(1:lx1,1:lx2,1:lx3)*gradlp1(1:lx1,1:lx2,1:lx3) &
  ...
  subroutine FBIheating(nn,Tn,ns,Ts,E2,E3,x,FBIproduction,FBIlossfactor)
    ...
    Bmagnitude=abs(Bmag_temp)    ! for Cartesian grid x%Bmag is negative in northern hemisphere
  ...
  subroutine srcsMomentum_neut(nn,vn1,vn2,vn3,Tn, &
    ...
    !NEUTRAL-ION collisions; note lsp == 7, ln == 4 (from module phys_consts)
    do isp=1,lsp
      do isp2=1,ln
        nu = 0._wp
...
!          nuneut * (vs3(1:lx1,1:lx2,1:lx3,isp) - vn3(1:lx1,1:lx2,1:lx3))
        ...
        momentumneut_source(1:lx1,1:lx2,1:lx3,1) = momentumneut_source(1:lx1,1:lx2,1:lx3,1) + &
          nn(1:lx1,1:lx2,1:lx3,isp2) * mn(isp2) * &
          nuneut * (vs1(1:lx1,1:lx2,1:lx3,isp) - (vn1(1:lx1,1:lx2,1:lx3)-vn1BG(1:lx1,1:lx2,1:lx3)))
        momentumneut_source(1:lx1,1:lx2,1:lx3,2) = momentumneut_source(1:lx1,1:lx2,1:lx3,2) + &
          nn(1:lx1,1:lx2,1:lx3,isp2) * mn(isp2) * &
          nuneut * (vs2(1:lx1,1:lx2,1:lx3,isp) - (vn2(1:lx1,1:lx2,1:lx3)-vn2BG(1:lx1,1:lx2,1:lx3)))
        momentumneut_source(1:lx1,1:lx2,1:lx3,3) = momentumneut_source(1:lx1,1:lx2,1:lx3,3) + &
          nn(1:lx1,1:lx2,1:lx3,isp2) * mn(isp2) * &
          nuneut * (vs3(1:lx1,1:lx2,1:lx3,isp) - (vn3(1:lx1,1:lx2,1:lx3)-vn3BG(1:lx1,1:lx2,1:lx3)))
...
!          +(vn3-vs3(1:lx1,1:lx2,1:lx3,isp))**2)*fact
        ...
        energyneut_source(1:lx1,1:lx2,1:lx3)=energyneut_source(1:lx1,1:lx2,1:lx3)+ &
          nn(1:lx1,1:lx2,1:lx3,isp2)*mn(isp2)*kB/(gamman(isp2)-1)*fact* &
          (Ts(1:lx1,1:lx2,1:lx3,isp) - (Tn(1:lx1,1:lx2,1:lx3)-TnBG(1:lx1,1:lx2,1:lx3)))

        !FRICTION
        fact=fact*mn(isp2)/3
        energyneut_source(1:lx1,1:lx2,1:lx3)=energyneut_source(1:lx1,1:lx2,1:lx3) + &
          nn(1:lx1,1:lx2,1:lx3,isp2)*mn(isp2)/(gamman(isp2)-1) &
          *( ((vn1(1:lx1,1:lx2,1:lx3)-vn1BG(1:lx1,1:lx2,1:lx3))-vs1(1:lx1,1:lx2,1:lx3,isp))**2 &
          +((vn2(1:lx1,1:lx2,1:lx3)-vn2BG(1:lx1,1:lx2,1:lx3))-vs2(1:lx1,1:lx2,1:lx3,isp))**2 &
          +((vn3(1:lx1,1:lx2,1:lx3)-vn3BG(1:lx1,1:lx2,1:lx3))-vs3(1:lx1,1:lx2,1:lx3,isp))**2)*fact
      end do
    end do
