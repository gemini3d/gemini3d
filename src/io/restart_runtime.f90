...
subroutine runtime_read(core,cfg,t)
  ...
  call f%read('/restart_runtime/input_sha256',digest)
  call f%close()
  if(schema/=1.or.any(layout/=[mpi_cfg%lid2,mpi_cfg%lid3])) error stop 'Incompatible runtime checkpoint layout/schema'
  if(digest/=input_digest) error stop 'Changed restart inputs: input SHA256 mismatch'
  if(index(prefix,'/')/=0.or.index(prefix,'..')/=0) error stop 'Invalid runtime checkpoint prefix'
  ...
  if(schema/=1.or.digest/=input_digest.or.iteration<1) error stop 'Invalid runtime state metadata'
...
subroutine runtime_save(cfg,ymd,ut,fluid,electro,vi1,vi2,vi3,iteration,neutral_time,tout,tglowout)
  ...
  call f%write('/schema',1);call f%write('/input_sha256',input_digest)
...
subroutine runtime_output_header(f)
  ...
  call f%write('/restart_runtime/schema',1)
