...
contains
  ...
  module procedure halo_end_23
    ...
    real(wp), dimension(:,:), allocatable :: outbuffer
    real(wp), dimension(:), allocatable :: inbuf_corner
    real(wp), dimension(:,:), allocatable :: inbuffer
    real(wp), dimension(:), allocatable :: outbuf_corner
    ...
    if (.not. (x3begin .and. x3end)) then
      ...
      allocate(outbuffer(lx1,lx2))
      outbuffer=param(:,:,1)
      call mpi_isend(outbuffer,lx1*lx2,mpi_realprec,idleft,tag,MPI_COMM_WORLD, requests(1))
  
      allocate(inbuffer(lx1,lx2))
    ...
    if (.not. (x2begin .and. x2end)) then
      ...
      !> force contiguous in memory
      allocate(outbuffer(lx1,lx3))
      outbuffer = param(:,1,:)
      call mpi_isend(outbuffer,lx1*lx3,mpi_realprec,iddown,tag,MPI_COMM_WORLD, requests(1))
  
      allocate(inbuffer(lx1,lx3))
    ...
    if (.not. (x2begin .and. x2end .and. x3begin .and. x3end)) then
      ...
      allocate(outbuf_corner(lx1))
      outbuf_corner=param(:,1,1)
      call mpi_isend(outbuf_corner,lx1,mpi_realprec,iddownleft,tag,MPI_COMM_WORLD, requests(1))
    ...
    if (x2end) paramtop=0
    !! add nothing on the end since no one is passing leftward to me, FIXME: need to account for periodic??? probably not if just
