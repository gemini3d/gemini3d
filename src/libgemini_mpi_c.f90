...
use gemini3d_mpi, only: mpisetup_in, mpiparms, &
 ...
 halo_allparams_in, RK2_prep_mpi_allspec_in, get_gavg_Tinf_in, clear_dneu_in, RK2_global_boundary_allspec_in, &
 halo_fluidvars_in
...
contains
  ...
  subroutine get_initial_state_C(cfgC,fluidvarsC,electrovarsC,intvarsC,xtype, &
                                   xC,UTsec,ymd,tdur) bind(C, name='get_initial_state_C')
  ...
  subroutine get_initial_drifts_C(cfgC, xtype,xC, fluidvarsC,fluidauxvarsC, &
                                    electrovarsC,intvarsC) bind(C, name='get_initial_drifts_C')
    type(C_PTR), intent(in) :: cfgC
    integer(C_INT), intent(in) :: xtype
    type(C_PTR), intent(in) :: xC
