...
!            ient=ient+1
  ...
  module procedure elliptic2D_static
    ...
    !number of neumann boundaries (x2 and x3) to used in this solve
    lneux2=0
    do ibnd=1,2
      if (flagsdirich(ibnd)==0) lneux2=lneux2+1
    end do
    ldirichx2=2-lneux2
    lneux3=0
    do ibnd=3,4
    ...
    !lent=5*(lx2-2)*(lx3-2) + 2*lx2 + 2*(lx3-2)        !static model has 5 diagonals for the FDE we are solving
    lent = 5*(lx2-2)*(lx3-2) + ldirichx2*lx3 + lneux2*2*lx3 + ldirichx3*(lx2-2) + lneux3*2*(lx2-2)
    allocate(ir(lent),ic(lent),M(lent),b(lPhi))
