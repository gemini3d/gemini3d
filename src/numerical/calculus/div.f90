...
!! NOTES:
!    A distinction is made between full grid and subgrid derivatives.  Each procedure will detect which is to be used based
!      on the sizes specified *for the x3 direction* for the derivative inputs.  If the x3 size for the derivative exceeds
!      the subgrid x3 size then we assume we are performing a full-grid derivative (in all directions).  Otherwise it is a
!      subgrid (local) derivative.  
